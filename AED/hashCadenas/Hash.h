...
ElemHT htRecupera(Hash h, int n, int id){ // hash, tamaño propuesto de la hash y un valor numerico del nombre de elemento
	Lista l = h[(id)%numPrimoSig(n)];

	while((!sonIgualesNumeros(valorString(getNombre(Cabeza(l))),id))&&(!esVacia(l)))
		l = Resto(l);
...
int estaEnHT(Hash h, int n, int id){ // hash, tamaño propuesto de la hash y un num atomico a buscar
	...
	if(htEsVacia(h)||esVacia(l)) // si esta vacia la tabla o el indice 
		...
		while(!esVacia(l)){
			if(sonIgualesNumeros(valorString(getNombre(Cabeza(l))),id))
				return 1;
			else
